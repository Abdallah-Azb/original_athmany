...
class BixolonService {
  ...
  printBixolonCashier(Accessory printer, Invoice invoice) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String baseUrl = prefs.getString('base_url');
    print("print BIXOLON cashier service");
    try {
      ...
      if (res == PosPrintResult.success) {
        ...
        Uint8List tableHeaderAs8List = await createImageFromWidget(
            _printInvoiceElements.tableHeader(width: 370),
        ...
        printerService.emptyLines(3);
        // printerService.image(eInvoiceAs8ListImage);
  ...
  printBixolonKitchen(Accessory printer, Invoice invoice) async {
    try {
      const PaperSize paper = PaperSize.mm80;
      final profile = await CapabilityProfile.load();
      final printerService = NetworkPrinter(paper, profile);
  ...
  printBixolonClosing(Accessory printer, List<PosTransaction> posTransaction,
    ...
    try {
      ...
      if (res == PosPrintResult.success) {
        ...
        // table header
        Uint8List tableHeaderAs8List = await createImageFromWidget(
            _printInvoiceElements.ClosingTableHeader(width: 370),
            logicalSize: Size(500, 500),
            imageSize: Size(680, 680));
        // Uint8List qrCodeAs8List = await createImageFromWidget(
        //     _printInvoiceElements.qrCode(invoice.offlineInvoice,
        ...
        Uint8List titleAs8List = await createImageFromWidget(
            Column(
              ...
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                _printInvoiceElements.ClosingTitle(),
              ],
            ),
            logicalSize: Size(500, 500),
            imageSize: Size(680, 680));
        ...
        int totalOfItems = 0;
        List<AnotherImage.Image> tableRowImages = [];
        for (PosTransaction item in posTransaction) {
          List<PosTransaction> itemOptions = posTransaction;
          double itemOptionsTotal = 0;
          // table item row
          Uint8List tableItemRowAs8List = await createImageFromWidget(
              _printInvoiceElements.TableClosingItem(
                  item.posInvoice, item.customer, item.grandTotal,
                  width: 370),
              logicalSize: Size(500, 500),
              imageSize: Size(680, 680));
          final AnotherImage.Image tableItemRowImage =
